...
def test_edit_expense_updates_fields(client, auth_cookie):
    ...
    dashboard = client.get("/dashboard", cookies=auth_cookie)
    # Extract the expense id from the edit link rendered on the dashboard-derived pages
    # (recent_transactions doesn't expose an edit link directly, so fetch via DB-independent route instead)
...
def test_delete_expense_removes_it(client, auth_cookie):
    ...
    delete_response = client.post(
        "/expenses/1/delete", cookies=auth_cookie, follow_redirects=False
    )
...
def test_editing_expense_owned_by_another_user_returns_404(client, auth_cookie):
    ...
    carol_login = client.post(
        "/login",
        data={"username": "carol", "password": "strongpass123"},
        follow_redirects=False,
    )
